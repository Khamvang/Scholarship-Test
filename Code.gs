...
function processSubmission(formObject) {
  ...
  var q24AnswerNormalized = (q24Answer || "").replace("ເຄີຍ", "").replace(/\s+/g, ' ').trim(); // allow both legacy and current wording for option D
  ...
  var q25AnswerNormalized = (q25Answer || "").replace(/\s+/g, ' ').trim();
  ...
  if (q25CorrectAnswers.indexOf(q25AnswerNormalized) !== -1) { q25Points = 5; }
